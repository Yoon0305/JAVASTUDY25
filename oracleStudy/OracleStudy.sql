...
-- EMP_ALTER 테이블의 EMPNO COLUMN의 데이터 타입을 NUMBER 형으로 바꾸고, 용량은 5자리로 하겠다.

ALTER TABLE EMP_ALTER DROP COLUMN TEL;

SELECT * FROM EMP_ALTER;

-- EMP_ALTER 테이블에서 DROP을 이용해서 TEL 열을 삭제.

ALTER TABLE EMP_ALTER DROP COLUMN COMM;

RENAME EMP_ALTER TO EMP_RENAME;

-- EMP_ALTER를 EMP_RENAME으로 테이블 이름 변경 

DESC EMP_ALTER;
-- 이름을 바꿔서 더이상 존재하지 않는 테이블이 됨.
...
SELECT * FROM EMP_RENAME;
-- EMP_RENAME 테이블 이하 내용 전체 삭제. (COLUMN 형식과 갯수만 남음)
...
CREATE TABLE EMP_HW ( EMPNO NUMBER(4), ENAME VARCHAR(10), JOB VARCHAR(9), MGR NUMBER(4), HIREDATE DATE DEFAULT SYSDATE , SAL NUMBER(7,2), COMM NUMBER(7,2), DEPTNO NUMBER(2) ); 
...
ALTER TABLE EMP_HW RENAME COLUMN REMAKR TO REMARK;

-- Q4

ALTER TABLE EMP_HW DROP COLUMN REMARK;

SELECT * FROM EMP_HW;

INSERT INTO EMP_HW (EMPNO,ENAME,JOB,MGR,HIREDATE,SAL,COMM,DEPTNO) SELECT (E.EMPNO,E.ENAME,E.JOB,E.MGR,E.HIREDATE,E.SAL,E.COMM,E.DEPTNO) FROM EMP E;
-- Q5,6 (미해결)
...
--TABLE_NOTNULL 이라는 테이블을 생성, ID, PW 의 데이터 타입, 크기, NOT NULL 제약 조건 설정 + TEL 은 제약조건 X

DESC TABLE_NOTNULL;

INSERT INTO TABLE_NOTNULL (LOGIN_ID, LOGIN_PWD, TEL) VALUES ('TEST_ID_01',NULL,'010-1234-1234');
-- LOGIN_PWD 에 NOT NULL이 걸려있어서 불가능.
...
-- SCOTT계정 소유의 USER_CONSTRAINTS 에 포함된 OWNER, CON.NAME, CON.TYPE, TABLE NAME 출력.

CREATE TABLE TABLE_NOTNULL2 ( LOGIN_ID VARCHAR(20) CONSTRAINT TBLNN2_LGNID_NN NOT NULL, LOGIN_PWD VARCHAR2(20) CONSTRAINT TBLNN2_LGNPW_NN NOT NULL, TEL VARCHAR2(20) );
-- CREATE TABLE (테이블 명) ( 자료명1 / 자료타입1 / CONSTRAINT (테이블 명) CONSTRAINT 종류, 자료명2 / 자료타입2 / CONSTRAINT (테이블 명) CONSTRAINT 종류 ) 

SELECT OWNER, CONSTRAINT_NAME CONSTRAINT_TYPE, TABLE_NAME FROM USER_CONSTRAINTS;
-- SYS_C~ 로 들어간 CONSTRAINT와 같은 제한 조건이지만 이름을 붙여서 제한조건이 어떤 주어에 어떤 형식으로 들어간 것인지 한눈에 파악하기 쉽게 함.

ALTER TABLE TABLE_NOTNULL MODIFY(TEL NOT NULL);
-- TEL 값에 이미 NULL 처리가 되었는데 NOT NULL을 통해 제약을 걸면 오류가 발생함.

UPDATE TABLE_NOTNULL SET TEL = '010-1234-1234' WHERE LOGIN_ID = 'TEST_ID_01';

SELECT * FROM TABLE_NOTNULL;

-- UPDATE 절을 이용해서 TABLE_NOTNULL의 TEST_ID_01의 TEL에 전화번호를 SET해서 넣음.

ALTER TABLE TABLE_NOTNULL MODIFY(TEL NOT NULL);

SELECT OWNER, CONSTRAINT_NAME, CONSTRAINT_TYPE, TABLE_NAME FROM USER_CONSTRAINTS;

-- USER_CONSTRAINTS 테이블이 갖고있는 정보를 조회해서 TABLE_NOTNULL에 NOT NULL CONSTRAINTS가 적용되었는지 확인.

-- 해당 방식의 최초 지정과의 차이 

-- 1) 최초 지정 방식 TEL / NUMBER(9) / NOT NULL 형태를 지님. 값은 예시임.

-- 2) 수정할때의 방식. ALTER TABLE TABLE_NOTNULL (MODIFY) 까지가 고정.

--      (1) COLUMN 명 변경 : ALTER ~ RENAME /  RENAME TEL COLUMN TO ** ;

--      (2) 타입 OR 크기 변경 :ALTER ~ MODIFY TEL VARCHAR(19)

--      (3) CONSTRAINTS 생성 :ALTER ~ MODIFY (TEL NOT NULL); -> 다른 MODIFY와 다르게 괄호로 변경 조건을 저장한다는 점에서 다름. 이 방식엔 CONSTRAINT 이름은 지정되지 않음.

--          (3)- A. 제약 조건에 이름을 넣어서 생성하는 방법 : ALTER ~ MODIFY(TEL CONSTRAINT TBLNN_TEL_NN NOT NULL); - CONSTRAINT 이하 /제약 조건명/ 제약조건.

--          (3)- B. 이미 생성된 제약 조건의 이름을 변경하는 방법 : ALTER ~ RENAME / RENAME CONSTRAINT /기존 제약 이름/ TO /변경할 제약 이름;  =>RENAME할때도 괄호가 들어가지 않는다.

--          (3)- C. 이미 생성된 제약 조건을 삭제하는 방법 : ALTER ~ DROP / DROP CONSTRAINT TBLNN2_TEL_NN; 
 
-- CONSTRAINT 새로 생성 또는 이름을 넣어 생성 시에만 괄호처리 된다.

---- ***** UNIQUE 시작

CREATE TABLE TABLE_UNIQUE ( LOGIN_ID VARCHAR2(20) UNIQUE, LOGIN_PW VARCHAR2(20) NOT NULL, TEL VARCHAR2(20) );

ALTER TABLE TABLE_UNIQUE RENAME COLUMN LOGIN_PW TO LOGIN_PWD;
-- TABLE_UNIQUE 라는 이름의 테이블 생성.

-- DETAIL 1 : LOGIN_ID VARCHAR2(20) UNIQUE

-- DETAIL 2 : LOGIN_PW VARCHAR2(20) NOT NULL

-- DETAIL 3 : TEL VARCHAR2(20) CONSTRAINT X

DESC TABLE_UNIQUE ;

SELECT OWNER, CONSTRAINT_NAME, CONSTRAINT_TYPE, TABLE_NAME FROM USER_CONSTRAINTS WHERE TABLE_NAME = 'TABLE_UNIQUE';
-- CONSTRAINT_TYPE 이 U(UNIQUE)인것을 확인 가능.

INSERT INTO TABLE_UNIQUE(LOGIN_ID, LOGIN_PWD, TEL) VALUES ('TEST_ID_01','PWD01','010-1234-1234');

SELECT * FROM TABLE_UNIQUE;

-- INSERT INTO 를 이용해서 테스트 ID,PW,TEL을 넣은 상황.

INSERT INTO TABLE_UNIQUE (LOGIN_ID, LOGIN_PWD, TEL) VALUES ('TEST_ID_01','PWD01','010-1234-1234');
-- 위와 동일한 조건을 넣었기 때문에 LOGIN_ID에 설정된 UNIQUE 무결성 CONSTRAINTS에 위배되어 삽입이 불가능.

INSERT INTO TABLE_UNIQUE (LOGIN_ID, LOGIN_PWD, TEL) VALUES('TEST_ID_02', 'PWD02','010-1234-5678');
-- TEST2 ID,PW,TEL 삽입.

UPDATE TABLE_UNIQUE SET LOGIN_PWD = 'PWD01' WHERE LOGIN_PWD =(SELECT LOGIN_PWD FROM TABLE_UNIQUE WHERE TEL = '010-1234-5678');

SELECT * FROM TABLE_UNIQUE;
-- LOGIN_PWD 열은 NOT NULL만 적용되어 있기 때문에 중복이 있어도 입력이 가능함.

INSERT INTO TABLE_UNIQUE(LOGIN_ID, LOGIN_PWD,TEL) VALUES (NULL,'PWD01','010-2345-6789');
-- UNIQUE의 성질을 알 수 있는 부분. UNIQUE는 중복은 허용하지만 NULL은 들어갈 수 있음.

-- 즉, UNIQUE 탭에 NULL이 여러개 존재 할 수 있다는 것.

-- UNIQUE, NOT NULL을 둘다 설정하지 않으면 PK로 사용하기 어려울 수 있다는 점이 있음.

UPDATE TABLE_UNIQUE SET LOGIN_ID = 'TEST_ID_01' WHERE LOGIN_ID IS NULL;
-- NULL처리된 ID 부분에 TEST_ID_01을 삽입하라는 명령문이나, UNIQUE 중복 금지 조건에 위배되어 실행되지 않음.
...
-- TABLE_UNIQUE 와 동일한 CONSTRAINT를 설정했으나, UNIQUE 명, NOT NULL 명을 입력한 모습.

ALTER TABLE TABLE_UNIQUE MODIFY (TEL UNIQUE);
-- 이미 생성된 테이블인 TABLE_UNIQUE 의 TEL COULMN에 대해 ()안의 새 CONSTRAINT를 설정하는 모습.
...
ALTER TABLE TABLE_UNIQUE2 DROP CONSTRAINT TBLUNQ_TEL_UNQ;

CREATE TABLE TABLE_PK ( LOGIN_ID VARCHAR2(20) PRIMARY KEY, LOGIN_PWD VARCHAR2(20) NOT NULL, TEL VARCHAR2(20) );

DESC TABLE_PK;
-- PK로 설정하는 CONSTRAINT 자체가 NULL임을 인정하지 않기 때문에 ID,PW 둘 다 NOT NULL 의 형태로 테이블이 생성되었음을 알 수 있음.

SELECT OWNER, CONSTRAINT_NAME, CONSTRAINT_TYPE, TABLE_NAME FROM USER_CONSTRAINTS WHERE TABLE_NAME LIKE 'TABLE_PK%';
-- NOT NULL 이 C / PRIMARY KEY 가 P 로 설정되어있는 것을 확인할 수 있음.

SELECT INDEX_NAME, TABLE_OWNER, TABLE_NAME FROM USER_INDEXES WHERE TABLE_NAME LIKE 'TABLE_PK%';
-- USER_INDEXES 에서 위에 생성한 TABLE_PK 의 자동생성된 INDEX를 확인할 수 있음.

SELECT * FROM TABLE_PK;

INSERT INTO TABLE_PK(LOGIN_ID, LOGIN_PWD,TEL) VALUES ('TEST_ID_01','PWD01','010-1234-5678');

-- CONSTRAINT 지정 COLUMN별.

CREATE TABLE TABLE_NAME( COL1 VARCHAR2(20) CONSTRAINT TBL_NM_PK PRIMARY KEY, COL2 VARCHAR2(20) CONSTRAINT TBL_NM_NN NOT NULL, COL3 VARCHAR2(20) );
...
--USER_CONSTRAINTS 의 테이블에서 EMP 와 DEPT 의 테이블 정보를 조회.

-- 두 테이블이 DEPTNO를 기준으로 참조하기 때문에 CONSTRAINT_TYPE : R 즉, REFERENCE 되어 있는 것을 알 수 있다.



-- TABLE 생성 및 COLUMN별 CONSTRAINT 입력 형식
...
-- COL3 NUMBER(10)
--  ... 
-- CONSTRAINT TBL_C1_PK PRIMARY KEY (COL1)
-- CONSTRAINT TBL_C2_NN NOT NULL (COL2)
-- CONSTRAINT TBL_C3_UNQ UNIQUE (COL3)
-- ...
-- );


-- TABLE 행 수정 : INSERT INTO ~
